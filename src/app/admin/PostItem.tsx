...
const SubmitButton = ({ label }: { label: string }) => {
	...
	return (
		<button
			type="submit"
			disabled={pending}
			className="inline-flex min-w-[120px] items-center justify-center rounded-full bg-emerald-600 px-4 py-2 font-[family-name:var(--font-geist-sans)] text-sm font-medium text-white transition hover:bg-emerald-500 disabled:cursor-not-allowed disabled:opacity-70 dark:bg-emerald-500 dark:text-gray-50 dark:hover:bg-emerald-400"
		>
			{pending ? (
				<>
...
const DeleteButton = ({ id }: { id: string }) => {
	...
	return (
		<button
			...
			onClick={onDelete}
			disabled={isPending}
			className="inline-flex items-center gap-2 rounded-full border border-red-200 bg-red-50 px-4 py-2 font-[family-name:var(--font-geist-sans)] text-sm text-red-600 transition hover:border-red-300 hover:bg-red-100 hover:text-red-700 disabled:cursor-not-allowed disabled:opacity-70 dark:border-red-500/40 dark:bg-red-950/40 dark:text-red-200 dark:hover:border-red-400 dark:hover:bg-red-900/60"
		>
			{isPending ? (
				<>
	...
	return (
		<div className="rounded-2xl border border-gray-200/70 bg-white/90 p-5 shadow-sm ring-1 ring-black/5 dark:border-gray-700/60 dark:bg-gray-900/70 dark:ring-white/5">
			<div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
				<div>
					<h3 className="font-[family-name:var(--font-geist-sans)] text-xl font-semibold text-gray-900 dark:text-gray-100">
						{post.title}
					</h3>
					<p className="mt-1 font-[family-name:var(--font-geist-sans)] text-sm text-gray-600 dark:text-gray-400">
						{post.excerpt}
					</p>
					<div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-gray-500 dark:text-gray-400">
						{post.publishedAt && <span>{new Date(post.publishedAt).toDateString()}</span>}
				...
				<div className="flex gap-2">
					<button
						...
						className="inline-flex items-center gap-2 rounded-full border border-gray-200 bg-white px-4 py-2 font-[family-name:var(--font-geist-sans)] text-sm text-gray-700 transition hover:border-gray-300 hover:text-gray-900 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200 dark:hover:border-gray-600 dark:hover:text-gray-50"
					>
